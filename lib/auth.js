...
const refreshUser = async () => {
  try {
    // First try to get the user from Supabase client
    const supabase = createClient();
    const { data: supabaseData } = await supabase.auth.getUser();
    
    if (supabaseData?.user) {
      console.log('User found in Supabase client:', supabaseData.user.email);
    ...
    // If no user in Supabase client, try the server
    const response = await fetch('http://localhost:3000/auth/user', {
      method: 'GET',
      credentials: 'include' // Important for cookies to be sent
    });
